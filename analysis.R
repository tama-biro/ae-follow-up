...
desc_data <- data %>%
  ...
  group_by(ID, condition, experiment) %>%
  summarize(post_low_c = 
            ...
            post_high_c = 
              ...
              mean(results[type == 'Control'])) %>%
  ungroup() %>%
  select(experiment, post_low_c, post_high_c)

psych::describeBy(desc_data$post_high_c, group = desc_data$experiment)
...
hist(data_t1$post_high_c[data_t1$condition == 'T2'])
shapiro.test(data_t1$post_high_c[data_t1$condition == 'T2'])

psych::describe((data_t1$post_high_c[data_t1$condition == 'T2']))

out_bc <- boxcox(data_t1$post_high_c+5, objective = 'Log-Likelihood',
                 lambda = seq(-10, 10))

which(out_bc$objective == max(out_bc$objective, na.rm = TRUE))

lmbda <- out_bc$lambda[14]

data_t1$post_high_bc <- boxcoxTransform(data_t1$post_high_c+5, lambda = lmbda)
...
pwr.t2n.test(n1 = 56, n2 = 31, d = 0.5, alternative = 'greater')

# Mixed model
data_int <- data %>%
  filter(type != 'Diversion' & results > 0 & experiment != 'S2') %>%
  group_by(ID) %>%
